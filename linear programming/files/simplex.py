...
def simplex_inner(lp, basis, tol):
    ...
    while max(matrix[0, 0: matrix_n-1] > tol):
        ...
        for i in range(1, matrix_m):
            if matrix[i, col] > tol and matrix[i, matrix_n - 1] > tol:
                if not row:
                    row = i
